...
import { useDashboardStore, type WidgetType, type DashboardLayout } from '@/shared/stores/dashboard.store'
...
const layoutGroups = computed(() => {
  const groups = [
    { key: 'zone-tile', label: 'Zone Tile', items: [] as DashboardLayout[] },
    { key: 'zone', label: 'Zone', items: [] as DashboardLayout[] },
    { key: 'cross-zone', label: 'Cross-Zone', items: [] as DashboardLayout[] },
    { key: 'sensor-detail', label: 'Sensor-Detail', items: [] as DashboardLayout[] },
    { key: 'other', label: 'Sonstige', items: [] as DashboardLayout[] },
  ]

  const getGroup = (scope?: string) => {
    if (scope === 'zone-tile') return groups[0]
    if (scope === 'zone') return groups[1]
    if (scope === 'cross-zone') return groups[2]
    if (scope === 'sensor-detail') return groups[3]
    return groups[4]
  }

  for (const layout of dashStore.layouts) {
    getGroup(layout.scope).items.push(layout)
  }

  for (const group of groups) {
    group.items.sort((a: DashboardLayout, b: DashboardLayout) => a.name.localeCompare(b.name))
  }
  ...
  <div class="dashboard-builder">
    ...
    <div class="dashboard-builder__toolbar">
        ...
        <div ref="layoutSelectorRef" class="dashboard-builder__layout-selector">
          ...
          <div v-if="showLayoutDropdown" class="dashboard-builder__layout-dropdown">
            ...
            <div
              ...
              :key="group.key"
              class="dashboard-builder__layout-group"
            >
              <div class="dashboard-builder__layout-group-title">{{ group.label }}</div>
              <div
              ...
              >
                ...
                <span v-if="layout.autoGenerated && (layout.scope === 'zone-tile' || layout.scope === 'zone')" class="dashboard-builder__auto-badge">Auto</span>
                <button
      ...
      <div class="dashboard-builder__toolbar-right">
        ...
        <!-- Target Configurator (progressive disclosure) -->
        <div v-if="dashStore.activeLayoutId && !isZoneTileLayout" class="dashboard-builder__target-wrapper">
          <button
        ...
        <div v-else-if="isZoneTileLayout" class="dashboard-builder__zone-tile-target-note">
          Zone-Tile: Anzeigeort ist fest auf Monitor-L1-Kachel.
        </div>
