...
function isLegacyAutoDualGaugeZoneTile(layout: DashboardLayout): boolean {
  if (!layout.autoGenerated || layout.scope !== 'zone-tile') return false
  if (layout.widgets.length !== 2) return false
  return layout.widgets.every(w => w.type === 'gauge')
}
...
/**
 * Ensures a zone-tile dashboard shell exists (scope zone-tile, optional Zusatz-Widgets im Editor).
 * Standard ohne Doppel-Gauges: Inhalt „Zoneinsight“ in {@link ZoneTileInsightBlock}.
