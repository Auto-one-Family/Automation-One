...
import { Pencil, ArrowLeft, Info } from 'lucide-vue-next'
...
const layout = computed(() =>
  dashStore.layouts.find(l => l.id === props.layoutId) ?? null
)

const isAutoGenerated = computed(() => layout.value?.autoGenerated === true)
...
function handleClaim() {
  ...
  dashStore.claimAutoLayout(layout.value.id)
}
</script>
...
<template>
  <div class="dashboard-viewer">
    ...
    <div v-if="isAutoGenerated" class="dashboard-viewer__auto-banner">
      <Info class="w-4 h-4" />
      <span>Dieses Dashboard wurde automatisch erstellt.</span>
      <button class="dashboard-viewer__claim-btn" @click="handleClaim">
        Übernehmen
      </button>
      <router-link
        v-if="layout"
        :to="{ name: 'editor-dashboard', params: { dashboardId: layout.id } }"
        class="dashboard-viewer__customize-btn"
      >
        Anpassen
      </router-link>
    </div>
    ...
    <div v-if="!layout" class="dashboard-viewer__empty">
      <p>Dashboard nicht gefunden.</p>
      <button class="dashboard-viewer__back-btn" @click="goBack">Zurück</button>
    </div>
...
.dashboard-viewer__auto-banner > svg {
  color: var(--color-iridescent-3);
  flex-shrink: 0;
}

.dashboard-viewer__claim-btn,
.dashboard-viewer__customize-btn {
...
.dashboard-viewer__claim-btn:hover {
  border-color: var(--color-iridescent-2);
...
.dashboard-viewer__customize-btn:hover {
  border-color: var(--glass-border-hover);
  color: var(--color-text-primary);
}
