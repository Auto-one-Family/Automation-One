...
test.describe('Logic Engine UI', () => {
  ...
  // ── AUT-128: Critical Badge & Degraded Pill ─────────────────────────

  test.describe('AUT-128: Critical & Degraded State', () => {
    let espId: string
    let ruleId: string
    ...
    test.beforeEach(async ({ page, request }) => {
      ...
      await createMockEspWithSensors(page, request, {
        ...
        zone_name: ZONE_NAME,
        sensors: [
          { gpio: 4, sensor_type: 'DS18B20', raw_value: 25.0, name: 'TempSensor' },
        ],
        actuators: [
          { gpio: 16, actuator_type: 'relay', name: 'Relay' },
        ],
    ...
    test('Regel mit is_critical=true zeigt Critical-Badge in RuleCard', async ({ page, request }) => {
      // Create a rule with is_critical: true via API
      const apiBase = `${getApiBase(page.url())}/api/v1`
      const token = await getToken(page)
      ...
      const response = await request.post(`${apiBase}/logic/rules`, {
        ...
        data: {
          name: 'E2E: Critical Badge Test',
          description: 'AUT-128 test rule with is_critical=true',
          enabled: true,
          is_critical: true,
          conditions: [
            {
      ...
      // The RuleCard should render with the critical badge
      const criticalBadge = page.locator('.rule-card__critical-badge')
      await expect(criticalBadge).toBeVisible({ timeout: 10000 })
    })

    // AUT-128: Benötigt WS-Mock-Infrastruktur aus AUT-130 für zuverlässige Injection
    test.skip('Regel zeigt Degraded-Pill nach rule_degraded WS-Event', async ({ _page, _request }) => {
      // AUT-128: Benötigt WS-Mock-Infrastruktur aus AUT-130
      //
      // Intended flow (once AUT-130 WS-mock is available):
      // 1. Create a rule via API
      // 2. Navigate to /logic
      // 3. Inject rule_degraded WebSocket event via page.evaluate()
      //    e.g.: window.__wsInject({ type: 'rule_degraded', data: { rule_id: ruleId, ... } })
      // 4. Assert: page.locator(`.rule-card__degraded-pill`).isVisible()
      //
      // Current blocker: no WS injection API available in test environment.
      // See: AUT-130 (WS-Mock-Infrastruktur)
    })
  })
