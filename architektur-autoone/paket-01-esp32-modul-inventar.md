...
| **FW-MOD-008** | `src/tasks/publish_queue.h/cpp` + `publish_queue_constants.h` + `publish_queue_policy.h` | **Core 1 → Core 0 Publish-Queue** (**10** Slots, `PUBLISH_QUEUE_SIZE`; ~22 kB Heap; AUT-481 P3; früher 8/AUT-362, historisch 15/AUT-344): `queuePublish()` von Core 1, adaptive Drain (`computeAdaptivePublishDrainBudget`: 1–2/Tick) via `processPublishQueue()` auf Core 0; `actuator/status` defer ab fill≥WATERMARK−1 | PublishRequest-Structs | MQTT-Publish-Aufrufe | RAM (Queue-Buffer) | mittel: Überfüllung führt zu Drop (non-blocking) | **hoch** | P1.2, P1.6 |
