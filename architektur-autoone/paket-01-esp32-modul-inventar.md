...
| **FW-MOD-008** | `src/tasks/publish_queue.h/cpp` | **Core 1 → Core 0 Publish-Queue** (**8** Slots, `PUBLISH_QUEUE_SIZE` in `publish_queue.h`, ~18 kB Heap; AUT-344 Reduktion von historisch 15; Planungs-MDs AUT-362): `queuePublish()` von Core 1, Drain via `processPublishQueue()` auf Core 0 | PublishRequest-Structs | MQTT-Publish-Aufrufe | RAM (Queue-Buffer) | mittel: Überfüllung führt zu Drop (non-blocking) | **hoch** | P1.2, P1.6 |
