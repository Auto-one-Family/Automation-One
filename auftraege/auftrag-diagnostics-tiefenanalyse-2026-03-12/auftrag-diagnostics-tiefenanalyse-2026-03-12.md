...
**Registrierung:** main.py Zeile X, api/v1/__init__.py Zeile Y
...
| mqtt | _check_mqtt | mqtt_manager.is_connected, ESPDevice |
| … | … | … |
...
**Datei:** `src/shared/stores/diagnostics.store.ts` (oder `El Frontend/src/…`)

**Schritte:**

1. **State:** currentReport, history, isRunning, runningCheck, …
2. **Actions:** runDiagnostic(), runCheck(name), loadHistory(), exportReport(id)
3. **runDiagnostic():** Welche API wird aufgerufen? POST /v1/diagnostics/run? Response-Handling?
...
|----------|------|-----|----------|
| runDiagnostic | POST | /v1/diagnostics/run | DiagnosticReport |
| … | … | … | … |
```
...
**Datei:** `src/components/system-monitor/DiagnoseTab.vue`
...
**Datei:** `src/components/system-monitor/ReportsTab.vue`
...
- `src/components/quick-action/QuickActionBall.vue`
...
**Datei:** `src/components/system-monitor/MonitorTabs.vue`

**Schritte:**

1. **Tabs:** Liste aller Tabs (Ereignisse, Logs, DB, MQTT, Health, Diagnostics, Reports)
...
**Tabs:** [7 Tabs]: events, logs, database, mqtt, health, diagnostics, reports
...
**Datei:** `src/services/logic/conditions/diagnostics_evaluator.py`
...
4. **Registrierung:** In Default-Liste der Condition-Evaluatoren? (Bekannt: NICHT in Default-Liste)
...
**Datei:** `src/services/logic/actions/diagnostics_executor.py`
...
5. **Registrierung:** In Default-Liste der Action-Executors? (Bekannt: NICHT in Default-Liste)
...
**Dateien:** `src/types/logic.ts`, Rule-Builder-Komponenten
...
2. **Datei:** `El Trabajante` — wo wird system/diagnostics publiziert?
3. **Intervall:** Wie oft? (z.B. mit Heartbeat)

**Server:**

4. **MQTT-Handler:** Welcher Handler empfängt system/diagnostics? Datei, Zeile
...
|-------|--------------|--------------|
| plugin_service=None | DiagnosticsService erhält plugin_service=None bei Init | A.2 prüfen |
...
`auftraege/DIAGNOSTICS-TIEFENANALYSE-BERICHT-2026-03-12.md`
...
- **Pfade:** Wenn die Struktur im auto-one Repo abweicht (z.B. `god_kaiser_server/` vs. `src/`), im Bericht die tatsächlichen Pfade dokumentieren.
