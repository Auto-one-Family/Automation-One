...
> **Letzte Aktualisierung:** 2026-04-22 (INC-2026-04-22 Klima-Forensik — 7 neue Issues AUT-109..115 + Delta-Kommentar auf AUT-68; **Cleanup 22.04. Nachmittag:** AUT-68 Scope-Drift behoben → repositioniert als "Phase 1 konservativ", AUT-121 neu "Heartbeat Metrics Split" ausgekoppelt, Reihenfolge AUT-111 vor AUT-110 in W18; **TM-Testlauf 22.04. 10:50:** AUT-68 Phase 1 **Iteration 1** umgesetzt — Commit `33ee862c` auf `auto-debugger/work`, nur Feld `seq` entfernt (1 Zeile in `mqtt_client.cpp:1371`), Verify-Gate hat 5 von 6 Kandidaten aus 07:40-Kommentar als Contract-relevant identifiziert → Umsetzung auf 1 Feld reduziert; User-Hand: `pio run -e esp32_dev` + Reflash + 10-min Live-Stresstest)
> **Aktiver Analyseauftrag (primär):** MQTT-Transport & Recovery Hardening (INC EA5484) — **19 Issues** (AUT-54..72 + AUT-121)
...
> **Forensik-Basis (NEU):** `docs/analysen/testfeld-live2-klima-forensik-2026-04-22.md` + `docs/analysen/testfeld-live2-klima-forensik-evidenz-2026-04-22.md` (in allen neuen Issues als Basis-Artefakt referenziert).
...
|---|-------|--------|----------------|-------------------|
| 1 | **MQTT-Transport & Recovery Hardening (INC EA5484)** | **19 Issues, 3x In Review, 16x Backlog; AUT-69 Firmware+Server live-verified, AUT-68 repositioniert + AUT-121 ausgekoppelt 22.04.; AUT-68 Iter1 committed 22.04. 10:50 (`33ee862c`) — wartet auf User-Hand-Build + Reflash** | MQTT-Transport & Recovery Hardening (INC EA5484) | **User-Hand: `pio run -e esp32_dev` + Reflash EA5484 + 10-min Live-Stresstest für AUT-68 Iter1 DoD. Danach AUT-72 (Memory-Leak) + AUT-71 (FE-Wiring) W1c/W2c ziehen. AUT-121 als nächstes Heartbeat-Paket nach AUT-68 Phase 1 Abschluss.** |
| 2 | **Testfeld Live-System 2 — Klima-Forensik (INC-2026-04-22)** | **7 Issues: AUT-109 (H8–H10 Sub von AUT-68) + AUT-110..115 (systemische Verbesserungen P0×2, P1×2, P2×2)** | Testfeld Live-System 2 — Klima-Forensik (INC-2026-04-22) | **verify-plan-Gates: AUT-111 (B-CRIT-01..04) zuerst, dann AUT-110 (B-ALRM-01..04) — Alerting konsumiert `is_critical`-Flag aus AUT-111; P0 vor W18-Start** |
...
- **AUT-109 bis AUT-115:** Testfeld Live-System 2 — Klima-Forensik (**INC-2026-04-22**, 7 Issues, **~22 SP**)
  ...
  - **Dispatch-Reihenfolge (W18..W20, Reihenfolge 22.04. korrigiert):** W18 = verify-plan-Gates **AUT-111 zuerst (Data-Model: `is_critical`-Flag + `escalation_policy` + Alembic-Migration)** → danach AUT-110 (Alerting konsumiert AUT-111-Schema) + AUT-109 Debug-Triage parallel. W19 = `server-dev`/`db-inspector` AUT-111 → `server-dev` AUT-110 + `server-dev` AUT-112. W20 = `frontend-dev` AUT-113/114/115 (nach AUT-110/111 Merge).
...
- **AUT-54 bis AUT-72 + AUT-121:** MQTT-Transport & Recovery Hardening — INC EA5484 (**19 Issues**, 4 RC-Cluster, ~55 SP)
  ...
  - **P3 Medium (4 SP):** AUT-70 (NTP-Boot-Entblockung Doku + Telemetrie, 1 SP), **AUT-121 (Heartbeat Metrics Split, NEU 22.04., 3 SP, parent=AUT-68)** — Forensik-/Delta-Counter aus Kern-Heartbeat auskoppeln, Hybrid-Transport empfohlen
...
- **INC-2026-04-22 Klima-Forensik (NEU, 22.04.):** 7 Issues angelegt — AUT-109 (EA-18 Sub-Issue unter AUT-68, H8–H10), AUT-110 (P0 Nacht-Regel-Alarm), AUT-111 (P0 Critical-Rule Degraded-Handling mit `is_critical` + `escalation_policy`), AUT-112 (P1 pg_dump Backup-Auth), AUT-113 (P1 HistoricalChart Gap-Marker), AUT-114 (P2 Conflict-Manager Logging/UX), AUT-115 (P2 Cockpit-Kachel, blockedBy AUT-110+AUT-111). Delta-Kommentar an AUT-68 (ID `e37ba8ed-7a89-4e03-b4a9-54212ba00435`) mit Verweis auf AUT-109. Basis-Artefakte: `docs/analysen/testfeld-live2-klima-forensik-2026-04-22.md` + `-evidenz-2026-04-22.md`.
- **Nächste Schritte INC-2026-04-22:** (1) verify-plan-Gate **AUT-111 zuerst** (B-CRIT-01..04, Data-Model-Migration + `is_critical`/`escalation_policy`) **dann AUT-110** (B-ALRM-01..04, Alerting konsumiert AUT-111-Schema) in W18 ziehen — P0, weil Safety-Lücke. (2) AUT-109 Debug-Triage per `esp32-debug` → `mqtt-debug` → `server-debug` (H8/H9/H10 durchprüfen). (3) Forensik-Dokumente `docs/analysen/testfeld-live2-klima-forensik-2026-04-22.md` + `-evidenz-2026-04-22.md` in Repo ablegen (User-Hand: vom TM-Chat-Input in Dateien speichern).
...
| INC EA5484 MQTT-Hardening | 0 (Forensik + Verifikation) | Multi-Agent (mqtt-dev primaer, esp32-dev + server-dev + frontend-dev) | **~55 SP** (19 Issues) — +3 AUT-69 +1 AUT-70 (Live-Evidence 20.04.), +2 AUT-71 +2 AUT-72 (Audit 20.04.), AUT-68 re-estimated 3→1 SP + AUT-121 +3 SP (Scope-Cleanup 22.04.) |
| INC-2026-04-22 Klima-Forensik | 0 (12-h-Forensik + Cross-System Analyse) | Multi-Agent (server-dev primaer für P0, frontend-dev für Widgets, esp32-debug/mqtt-debug/server-debug für AUT-109) | **~22 SP** (7 Issues: 3+5+5+3+2+2+2) — P0×2 (AUT-110/111), P1×3 (AUT-109/112/113), P2×2 (AUT-114/115) |
