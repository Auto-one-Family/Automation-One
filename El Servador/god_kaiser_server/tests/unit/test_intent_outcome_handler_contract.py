...
@pytest.mark.asyncio
async def test_missing_outcome_is_defaulted_to_unknown_and_persisted():
    """AUT-304: Pre-fix firmware can omit ``outcome`` under ArduinoJson budget pressure.
    Handler must default to 'unknown' and flag CONTRACT_MISSING_OUTCOME instead of
    permanently dropping the payload."""
    handler = IntentOutcomeHandler()
    payload = {
        "intent_id": "emergency_668237_84",
        "correlation_id": "corr_668238_74",
        "flow": "command",
        # ``outcome`` intentionally absent — mirrors pre-AUT-304 firmware bug
        "ts": 1778279439,
        "seq": 113,
    }
    ...
    contract_repo.upsert_outcome = AsyncMock(
        return_value=(
            SimpleNamespace(
                ...
                correlation_id="corr_668238_74",
                flow="command",
                code="CONTRACT_MISSING_OUTCOME",
                reason="Contract violation: missing outcome",
                retryable=False,
                generation=0,
                seq=113,
                epoch=0,
    ...
    with (
        patch(
            "src.mqtt.handlers.intent_outcome_handler.TopicBuilder.parse_intent_outcome_topic",
            return_value={"esp_id": "ESP_698EB4"},
        ),
        patch(
            "src.mqtt.handlers.intent_outcome_handler.resilient_session",
            fake_resilient_session,
        ...
        result = await handler.handle_intent_outcome(
            "kaiser/god/esp/ESP_698EB4/system/intent_outcome",
            payload,
        )

    assert result is True
    assert captured_payloads, "Payload must be persisted, not dropped"
    # outcome is canonicalized from 'unknown' — the important check is the violation code
    assert captured_payloads[0]["code"] == "CONTRACT_MISSING_OUTCOME"
    assert captured_payloads[0]["retryable"] is False
