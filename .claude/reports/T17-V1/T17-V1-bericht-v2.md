# T17-V1 Backend + Data Integrity — Verifikationsbericht (V2)

**Datum:** 2026-03-10 14:30
**Gesamt:** 23/25 PASS, 1 FAIL, 1 PARTIAL

## Zusammenfassung

Fix-V (Notification Fingerprint), Fix-W (ESP Health Resilience), Fix-X (Alert + Health), Fix-T (DB Infrastructure), Fix-FW/Fix-LOG (Config-Mismatch Loop) und 6.0 (SHT31 Dedup) sind vollstaendig verifiziert und funktional. Fix-U (Actuator Offline Lifecycle) hat eine Luecke: Actuator-Kommandos werden nicht bei Offline-ESPs abgelehnt — kein Online-Check in der Command-Pipeline. V1-21 (Actuator State Reset) konnte nur Code-seitig verifiziert werden — Live-Test erfordert ESP-Disconnect.

---

## Testergebnisse

### Block A: Notification Fingerprint Propagation (Fix-V)
...
### Block B: ESP Health Resilience (Fix-W)
...
### Block C: Alert + Health Endpoint (Fix-X)
...
### Block D: DB Infrastructure (Fix-T)
...
-- Result: uq_sensor_data_esp_gpio_type_timestamp
```
...
### Block E: Config-Mismatch Loop (Fix-FW + Fix-LOG)
...
**Status:** PASS
**Evidenz:** `docker compose logs --since 1h el-servador | grep -ic "config.*mismatch\|config push"` liefert **2 Eintraege** in 1 Stunde. Level ist INFO (nicht WARNING). Cooldown greift — deutlich unter 480/h.
```
...
### Block F: Actuator Offline Lifecycle (Fix-U)

### V1-21 — Actuator State Reset bei Offline-Transition
**Status:** PARTIAL
**Evidenz:** Zwei Pfade implementiert und Code-verifiziert:
...
**Notizen:** Code korrekt implementiert. Live-Test (ESP physisch trennen und DB-State pruefen) konnte nicht durchgefuehrt werden — erfordert manuellen ESP-Disconnect.
...
-- Result: (0 rows)
```
Keine stale Actuator-States.

---

### Block G: SHT31 Duplikat-Fix (6.0)
...
FROM sensor_data WHERE sensor_type LIKE 'sht31%' AND timestamp > NOW() - INTERVAL '24 hours'
GROUP BY esp_id, gpio, sensor_type, timestamp HAVING COUNT(*) > 1;
-- Result: (0 rows)
```
...
-- Result: sht31_temp, sht31_humidity
```
...
| ID | Problem | Schwere | Status | Empfehlung |
|----|---------|---------|--------|------------|
| V1-21 | Actuator State Reset nur Code-verifiziert, kein Live-Test | MEDIUM | PARTIAL | Manueller ESP-Disconnect-Test bei naechster Hardware-Session |
| V1-22 | Actuator-Kommandos werden nicht bei Offline-ESPs abgelehnt | HIGH | FAIL | Online-Check in `actuator_service.py` oder `safety_service.py` vor MQTT-Publish |
