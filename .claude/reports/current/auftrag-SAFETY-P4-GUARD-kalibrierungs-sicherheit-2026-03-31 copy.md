...
**Status:** IMPLEMENTIERT (16 Tests PASS, Build OK). **Nachschaerfung noetig:** soil_moisture Alias-Luecke → siehe `auftrag-ANALYSE-P4-GUARD-sensortyp-normalisierung-2026-03-31.md`
...
**Datei:** `El Servador/god_kaiser_server/tests/unit/test_config_builder_offline_rules.py` (16 Tests verifiziert: 11x TestExtractOfflineRuleUnit + 5x TestBuildOfflineRulesAsync).
...
def test_offline_rule_skips_ph_sensor():
    ...
    rule = create_test_logic_rule(
        condition_sensor_type="ph",
        condition_gpio=34,  # ADC1-Pin
        actuator_gpio=27
    )
    result = config_builder._extract_offline_rule(rule, esp_id)
...
> **Hinweis Logging-Tag:** Der Auftrag zeigt `LOG_W("OFFLINE", ...)`. Die tatsaechliche Implementierung nutzt `"SAFETY-P4"` als Tag — funktional identisch, konsistenter mit Projekt-Namensschema.
...
- [ ] `_extract_offline_rule()` gibt `None` zurueck fuer sensor_type `ph`, `ec`, `moisture`
- [ ] Warning-Log erscheint fuer jede gefilterte Rule (mit Logic-Rule-ID + sensor_type)
- [ ] Alle 16 bestehenden Offline-Rule-Tests laufen weiterhin PASS (11x Unit + 5x Async)
- [ ] 3 neue Tests fuer den Analog-Filter: PASS
...
- [ ] Build kompiliert ohne Warnings — Dev-Build: `cd "El Trabajante" && pio run -e esp32_dev`, Hardware-Build: `cd "El Trabajante" && pio run -e seeed`
...
- **ANALYSE-P4-GUARD-sensortyp-normalisierung:** Nachschaerfung fuer den GUARD — `soil_moisture` Alias umgeht den `split("_")[0]`-Filter. Empfehlung: `normalize_sensor_type()` VOR dem Filter anwenden (Server) + `requiresCalibration()` um Aliase erweitern (Firmware). Zusaetzlich: applyLocalConversion() fuer light/CO2/flow pruefen, Frontend sensor_type Herkunft klaeren.
