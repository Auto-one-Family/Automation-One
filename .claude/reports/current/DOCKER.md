...
├── docker/               # Docker configs (grafana, loki, prometheus, promtail, pgadmin, postgres, mosquitto)
...
├── docker-compose.yml    # Main Docker Compose (9 services)
...
### docker-compose.yml (Main - 9 Services)
...
| 5 | **pgadmin** | dpage/pgadmin4:8.14 | automationone-pgadmin | 5050:80 | postgres (healthy) | devtools | 256M limit, 128M reserved |
| 6 | **loki** | grafana/loki:3.4 | automationone-loki | 3100:3100 | - | monitoring | 512M limit, 256M reserved |
| 7 | **promtail** | grafana/promtail:3.4 | automationone-promtail | - | loki (healthy) | monitoring | 128M limit, 64M reserved |
| 8 | **prometheus** | prom/prometheus:v3.2.1 | automationone-prometheus | 9090:9090 | el-servador (healthy) | monitoring | 512M limit, 256M reserved |
| 9 | **grafana** | grafana/grafana:11.5.2 | automationone-grafana | 3000:3000 | prometheus (healthy), loki (healthy) | monitoring | 256M limit, 128M reserved |
...
| Prometheus | prom/prometheus:v3.2.1 | 9090 | Metrics collection (scrapes el-servador:8000/metrics, self:9090) |
...
make watch       - Compose Watch (auto-sync)
```
...
**CI:**
```
make ci-up       - Start CI stack (tmpfs postgres)
make ci-down     - Stop CI stack, remove volumes
make ci-test     - Start CI services for tests
make ci-logs     - Follow CI stack logs
```
...
make e2e-report  - HTML report
make e2e-debug   - Debug mode
make e2e-codegen - Code generator
```
...
- Docker configs (grafana, loki, pgadmin, prometheus, promtail)
- `docs/`
...
| 5050 | pgAdmin | automationone-pgadmin | HTTP | Database admin UI |
...
# 4. Start dev tools (optional)
make devtools-up   # pgAdmin at :5050
