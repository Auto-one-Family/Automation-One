...
> **STATUS (verifiziert 2026-03-31):** ALLE 3 BUGS IMPLEMENTIERT ✅
> - Bug-1 → Commit `bedd356`: fix(esp32): Bug-1 — MQTT disconnect respects offline rules before safe state
> - Bug-3 → Commit `3e22fcc`: fix(esp32): Bug-3 — P1 ACK timeout delegates to P4 when offline rules exist
> - Bug-2 → config_builder.py enthält alle 4 logger.warning()-Stellen + Summary-Log (in working tree, noch kein Commit)
...
**SAFETY-P1 (ACK-Timeout):** ESP sendet Heartbeat alle **60s** (`HEARTBEAT_INTERVAL_MS = 60000` in `mqtt_client.h`). Server antwortet mit ACK (**QoS 0**, erst NACH DB-Arbeit). Wenn 120s kein ACK (`SERVER_ACK_TIMEOUT_MS`) → Aktoren abgeschaltet. Greift NUR wenn MQTT noch verbunden ist (`mqttClient.isConnected()` in `checkServerAckTimeout()`). P1 triggert P4 via `offlineModeManager.onDisconnect()`. Szenario: Server-Prozess crashed, Broker laeuft weiter. **Achtung:** Bei 60s Heartbeat und 120s Timeout sind nur ~2 ausgebliebene ACKs bis zum Trigger — ein verlorener QoS-0-ACK plus ein verzoegerter Heartbeat kann P1 bei funktionierendem Server ausloesen (False Positive).
...
- [x] Bei MQTT-Disconnect: Kein `PumpActuator GPIO X OFF` innerhalb der ersten 30s
- [x] Grace Period wird respektiert, Aktor bleibt im aktuellen Zustand
- [x] Nach 30s: OFFLINE_ACTIVE → Safety-Task evaluiert Rules → Aktor reagiert korrekt

**Ohne Offline-Rules (offline_rule_count_ == 0):**
- [x] Bei MQTT-Disconnect: Aktor wird SOFORT auf safe state gesetzt (kein Grace, kein Warten)
- [x] Log zeigt "no offline rules, safe state immediately"

**Allgemein:**
- [x] P4 State-Machine laeuft in beiden Faellen (`onDisconnect()` wird IMMER aufgerufen)
- [x] Fix 1c Defense-in-Depth: Falls P4 mit 0 Rules OFFLINE_ACTIVE erreicht → bestaetigt safe state (Aktoren sind bereits AUS)
- [x] Emergency-Stop (`NOTIFY_EMERGENCY_STOP`) funktioniert weiterhin sofort
- [x] Keine neuen `#ifndef WOKWI_SIMULATION` Guards noetig (bestehende Guards betreffen nur WDT-Calls)
...
**Mit Offline-Rules:**
- [x] Bei Server-Stop: P1-Log mit "delegating to P4", **KEIN** Aktor-OFF durch P1
- [x] Aktor bleibt 30s unveraendert (Grace Period)
- [x] Nach 30s: P4 evaluiert Offline-Rules
- [x] Kein 50s "falsches AUS" Fenster mehr

**Ohne Offline-Rules:**
- [x] Bei Server-Stop: P1 schaltet sofort ab (wie bisher)
- [x] Log zeigt "no offline rules, safe state immediately"
...
- [x] Server-Log zeigt fuer jede uebersprungene Rule den Grund
- [x] `Built X offline rules for ESP` Log am Ende
- [x] Kein doppeltes Logging bei den 3 bereits abgedeckten Stellen
- [ ] ESP Serial zeigt: `[CONFIG] Received X offline rules` mit X > 0 (Achtung: TAG ist `SAFETY-P4` aber Message-Prefix ist `[CONFIG]`) — Laufzeit-Test noch offen
- [ ] ESP Serial zeigt: `[CONFIG] Saved X offline rules to NVS` — Laufzeit-Test noch offen
...
| Datei | Bug | Zeile (IST) | Status | Aenderung |
|-------|-----|-------------|--------|-----------|
| `src/tasks/safety_task.cpp` | Bug-1 | 61-73 | ✅ COMMITTED (bedd356) | Rule-Count-Guard vor `setAllActuatorsToSafeState()` |
| `src/services/communication/mqtt_client.cpp` | Bug-1 | 999-1011 | ✅ COMMITTED (bedd356) | Rule-Count-Guard in `handleDisconnection()` |
| `src/services/safety/offline_mode_manager.cpp` | Bug-1 | 341-349 | ✅ COMMITTED (bedd356) | Defense-in-Depth: 0-Rules → safe state in `activateOfflineMode()` |
| `offline_mode_manager.h` | Bug-1 | 82 | ✅ BEREITS VORHANDEN | `getOfflineRuleCount()` war schon public |
| `src/main.cpp` | Bug-3 | 2332-2343 | ✅ COMMITTED (3e22fcc) | Rule-Count-Guard in `checkServerAckTimeout()` |
| `god_kaiser_server/src/services/config_builder.py` | Bug-2 | 397-402, 413-423, 461-466, 487-489, 354-359 | ✅ IN WORKING TREE (noch kein Commit) | 4x `logger.warning()` + Summary-Log `logger.info()` |

**Reihenfolge + Verifikation:**

| Schritt | Bug | Build/Test | Status |
|---------|-----|-----------|--------|
| 1 | Bug-1 (3 Dateien) | `pio run -e esp32_dev` + `pio run -e seeed_xiao_esp32c3` | ✅ Committed |
| 2 | Bug-3 (main.cpp) | `pio run -e esp32_dev` + `pio run -e seeed_xiao_esp32c3` | ✅ Committed |
| 3 | Bug-2 (config_builder.py) | `pytest tests/unit/test_config_builder_offline_rules.py` | ⚠️ Code implementiert, Commit + Test ausstehend |
