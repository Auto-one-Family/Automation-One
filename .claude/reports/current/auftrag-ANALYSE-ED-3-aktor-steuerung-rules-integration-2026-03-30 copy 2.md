...
### A1: condition_index hardcoded auf 0 (N1 — HOCH)
...
### A3: DiagnosticsConditionEvaluator fehlt in logic_service.py (N4 — MITTEL)
...
### A4: GPIO int()-Coercion im HysteresisConditionEvaluator (N5 — MITTEL)
...
### B1: extractSensorConditions() ignoriert Hysterese-Typ (N6 — MITTEL)
...
### C1: ESP subscribed Aktor-Commands mit QoS 0 (N8 — NIEDRIG)

**Datei:** `El Trabajante/src/main.cpp` (Zeile ~825) oder `El Trabajante/src/communication/mqtt_client.cpp`
...
### D1: HysteresisConditionEvaluator Unit-Tests (KRITISCH — noch nicht vorhanden)

**Datei (neu):** `El Servador/tests/unit/logic/test_hysteresis_evaluator.py`
...
### D2: Hysterese-State-Persistenz-Test (N2 Fix)

**Datei (neu):** `El Servador/tests/integration/logic/test_hysteresis_state_persistence.py`
...
**Datei (neu oder ergaenzen):** `El Servador/tests/unit/logic/test_compound_hysteresis.py`
...
**Datei:** `El Frontend/src/tests/unit/logic.test.ts` (oder neue Datei)
