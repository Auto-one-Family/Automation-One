...
```python
# scripts/cleanup_orphaned_configs.py (oder als Management-Command)
"""
...
from db.models.esp_device import ESPDevice

async def cleanup_orphaned_sensor_configs(session):
...
Falls der Health-Check ueber sensor_configs iteriert, muss er Configs von deleted Devices ausschliessen:

```python
# health_check.py — Filter hinzufuegen
# VORHER:
# all_configs = await sensor_repo.get_all()

# NACHHER:
# all_configs = await sensor_repo.get_all_active()
# (WHERE esp_device.deleted_at IS NULL — via JOIN oder Subquery)
```

Dieser Filter verhindert, dass zukuenftige Orphans (falls Cascade-Delete je versagt) den Health-Check stoeren. In T10 Phase 11 wurde bestaetigte, dass der Health-Check nach Device-Delete die geloeschten Devices nicht mehr prueft — dieser Filter ist eine ZUSAETZLICHE Sicherheit.
...
| Datei | Aenderung |
|-------|-----------|
| `scripts/cleanup_orphaned_configs.py` | NEU — einmaliges Cleanup-Script |
| `health_check.py` (optional) | Filter fuer deleted_at Devices |
