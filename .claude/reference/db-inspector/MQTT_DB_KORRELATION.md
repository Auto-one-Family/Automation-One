...
|12 | `kaiser/{kaiser}/esp/{esp_id}/system/heartbeat` | Heartbeat-Payload inkl. **`hardware_type`** (z. B. `ESP32_S3_DEVKITC1`) | `esp_heartbeat_logs` (+ `esp_devices.last_seen`, **`esp_devices.hardware_type`** Update bei Abweichung, Spiegel in `device_metadata`) | `topics.py` `build_heartbeat_topic`; `src/mqtt/handlers/heartbeat_handler.py` |
