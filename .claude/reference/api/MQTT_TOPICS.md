...
> **Version:** 2.32 | **Aktualisiert:** 2026-05-25
...
**3012 MQTT-Publish-Kontext (AUT-454, additiv):** Bei `error_code=3012` kann `context`
zusätzlich folgende Diagnosefelder enthalten: `topic` (string), `gpio` (int),
`sensor_type` (string), `reason_class` (string). `reason_class` ist kanonisch einer
von `cb_open`, `queue_shed`, `outbox_full`, `transport_error`.
...
**Intent-Metadaten im Server-Payload:** Primär top-level (`intent_id`, `correlation_id`, `generation`, …). Optional zusätzlich unter `data.*` gespiegelt — Firmware liest top-level zuerst, dann `data` (`intent_contract.cpp`). **Config-Ingress:** Wenn Filter-Parse `correlation_id` verpasst, fuellt `tryWireFillIntentCorrelation()` (strstr, kein Heap) aus dem Roh-JSON nach — nur Config-Pfade (`main.cpp`, `config_update_queue.cpp`); Aktor-Hotpath bleibt bei `StaticJsonDocument<1024>`.
...
- Fehlende `correlation_id` wird im Firmware-Config-Pfad strict als Vertragsfehler emittiert (`CONTRACT_MISSING_CORRELATION` / `CONTRACT_CORRELATION_MISSING`); kein generischer `fw_*`-Fallback im terminalen Pfad. Vor dem Fehler: `tryWireFillIntentCorrelation()` (Wire-Copy aus Roh-JSON, config-only).
