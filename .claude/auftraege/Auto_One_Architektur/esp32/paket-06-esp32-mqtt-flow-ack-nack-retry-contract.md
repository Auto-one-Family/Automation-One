...
|---|---|---|---|---|
| FW-NET-FLOW-001 | Core1 Publish -> Core0 -> Broker (ESP-IDF) | `g_publish_queue` (**10**, `PUBLISH_QUEUE_SIZE` in `publish_queue_constants.h`; früher 8/AUT-362; AUT-481 P3) + ESP-IDF outbox (16384 B via `sdkconfig.defaults`) | Queue full: Drop + ggf. `publishIntentOutcome` (`publish_queue.cpp`); Shed ab fill≥5; Drain (`processPublishQueue`) adaptive 1–2/Tick via `computeAdaptivePublishDrainBudget`; msg_id<0 → `OUTBOX_FULL`/`EXECUTE_FAIL`, kritische Messages bis 3x re-queue | **Drop nur nach definiertem Retry/Outcome-Pfad (nicht mehr teil-silent)** |
