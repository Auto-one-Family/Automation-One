...
|---|---|---|---|---|
| FW-NET-FLOW-001 | Core1 Publish -> Core0 -> Broker (ESP-IDF) | `g_publish_queue` (**8**, `PUBLISH_QUEUE_SIZE` in `publish_queue.h`; historisch 15, AUT-344; Doku AUT-362) + ESP-IDF outbox | Queue full: Drop + ggf. `publishIntentOutcome` (`publish_queue.cpp`); Drain (`processPublishQueue`) wertet `esp_mqtt_client_publish` aus: msg_id<0 → `OUTBOX_FULL`/`EXECUTE_FAIL`, kritische Messages bis 3x re-queue, sonst `publishIntentOutcome` | **Drop nur nach definiertem Retry/Outcome-Pfad (nicht mehr teil-silent)** |
