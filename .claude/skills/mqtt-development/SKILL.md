...
| Firmware Publish-Queue (M3) | `El Trabajante/src/tasks/publish_queue_constants.h`; `publish_queue_policy.h`; `publish_queue.cpp`; `communication_task.cpp` | Core-1→Core-0 Queue (10 Slots, Shed 5); adaptive Drain 1–2/Tick; `system/queue_pressure` Hysterese (AUT-481 P3). | ja |
...
| 14a | `system/queue_pressure` | ESP→Server | 0 | `buildQueuePressureTopic()` *(PKG-01a)* | `TopicBuilder.build_queue_pressure_topic` *(PKG-01)* |
